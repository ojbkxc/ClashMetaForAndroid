...
object RootHelper {
    ...
    private var cachedDnsUid: Int = -1
    ...
    /**
     * 获取 DNS 进程 UID（系统 DNS resolver）
     * Android 的 DNS resolver 通常运行在 UID 1051 (dns) 或与应用相同的 UID
     */
    private fun getDnsUid(): Int {
        if (cachedDnsUid > 0) return cachedDnsUid
        // 尝试获取 netd 的 UID
        val (code, output) = RootChecker.execute("ps -A | grep netd")
        if (code == 0 && output.isNotBlank()) {
            // netd 通常运行在 root (UID 0)，不需要特殊处理
            // DNS 查询实际由应用发起，UID 就是应用的 UID
            cachedDnsUid = getAppUid()
            return cachedDnsUid
        }
        cachedDnsUid = getAppUid()
        return cachedDnsUid
    ...
    /**
     * 创建或清理 iptables chain（参考 SagerNet 方案）
     * 先删除旧 chain 再创建，避免 "Chain already exists" 错误
     */
    private fun ensureChain(table: String, chain: String): Boolean {
        ...
        // 创建新 chain
        val (code, output) = RootChecker.execute("iptables -t $table -N $chain")
        return code == 0
    }
    ...
    private fun isTProxySupported(): Boolean {
        val (code, _) = RootChecker.execute(
            "iptables -t mangle -N CLASH_TPROXY_TEST 2>/dev/null ; " +
            "iptables -t mangle -A CLASH_TPROXY_TEST -p tcp -j TPROXY --tproxy-mark 0x1/0x1 --on-port 1 2>/dev/null ; " +
            "iptables -t mangle -F CLASH_TPROXY_TEST 2>/dev/null ; " +
            "iptables -t mangle -X CLASH_TPROXY_TEST 2>/dev/null"
        )
        return code == 0
    }
     ...
     * 参考 SagerNet 的 dokodemo-door + tproxy 配置
     */
    private fun applyTProxy(proxyUid: Int): Pair<Boolean, String> {
        data class Step(val cmd: String, val desc: String)
        val steps = listOf(
            // 创建 chain
            Step("iptables -t mangle -N $CHAIN_META", "创建 chain"),
            // 跳过代理进程自身流量（参考 SagerNet 的 inboundTag 方案）
            ...
            // TPROXY 规则（带 fwmark 标记，参考 SagerNet 的 tproxy mark 0x1/0x1）
     ...
     * 参考 SagerNet 的 REDIRECT 模式（transproxyMode = 0）
     * 使用 iptables nat 表的 REDIRECT 目标，不需要路由规则
     */
    private fun applyRedirect(proxyUid: Int): Pair<Boolean, String> {
        data class Step(val cmd: String, val desc: String)
        val steps = listOf(
            // 创建 chain
            Step("iptables -t nat -N $CHAIN_META", "创建 chain"),
            // 跳过代理进程自身流量
    ...
    suspend fun applyLockBackground(): Pair<Boolean, String> {
        ...
        data class Step(val cmd: String, val desc: String)
        val steps = listOf(
            Step("iptables -t mangle -N $CHAIN_LOCK_BG", "创建 chain"),
    ...
    suspend fun applyDnsHijack(): Pair<Boolean, String> {
        ...
        // 动态获取应用 UID，跳过代理进程的 DNS 查询
        val proxyUid = getAppUid()

        data class Step(val cmd: String, val desc: String)
        val steps = mutableListOf(
            Step("iptables -t nat -N $CHAIN_DNS_HIJACK", "创建 DNS 劫持 chain"),
